...
from bot import aria2, download_dict_lock, download_dict, BASE_URL, LOGGER
...
def add_aria2c_download(link: str, path, listener, filename, auth, select, ua, sp_con):
    if is_magnet(link):
        download = aria2.add_magnet(link, {'dir': path})
    else:
        download = aria2.add_uris([link], {'dir': path, 'out': filename, 'header': f"authorization: {auth}", 'header': f"user-agent: {ua}", 'split': sp_con})
