...
class GoogleDriveHelper:
    ...
    def __init__(self, name=None, listener=None):
        ...
        self.__file_downloaded_bytes = 0
    ...
    def _on_upload_progress(self):
        if self.status is not None:
            self.__chunk_size = self.status.total_size * self.status.progress() - self.__file_uploaded_bytes
            self.__file_uploaded_bytes = self.status.total_size * self.status.progress()
            self.uploaded_bytes += self.__chunk_size
    ...
    def _on_download_progress(self):
        if self.dstatus is not None:
            self.__chunk_size = self.dstatus.total_size * self.dstatus.progress() - self.__file_downloaded_bytes
            self.__file_downloaded_bytes = self.dstatus.total_size * self.dstatus.progress()
            self.downloaded_bytes += self.__chunk_size
