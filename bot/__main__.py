...
from .modules import authorize, cancel_mirror, mirror_status, mirror, clone, watch, shell, delete, leech_settings, search, rss, rcdrive, info, gddrive, bt_select
...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("Repo", "https://github.com/code0o0/mirror-leech-telegram-bot.git")
    buttons.buildbutton("Report Group", "https://t.me")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
This bot can mirror all your links to the drive you want!
...
help = telegraph.create_page(title='Mirror-Leech-Bot Help', content=help_string_telegraph, )["path"]
...
botcmds = [
        (f'{BotCommands.MirrorCommand}', 'Mirror link n-name:xx pswd:xx[zip/unzip] ua:xx sp_con:xx'),
        (f'{BotCommands.ZipMirrorCommand}','Mirror and upload as zip'),
        (f'{BotCommands.UnzipMirrorCommand}','Mirror and extract files'),
        (f'{BotCommands.QbMirrorCommand}','Mirror torrent using qBittorrent'),
        (f'{BotCommands.QbZipMirrorCommand}','Mirror torrent and upload as zip using qb'),
        (f'{BotCommands.QbUnzipMirrorCommand}','Mirror torrent and extract files using qb'),
        (f'{BotCommands.BtSelectCommand}','Select files from already added qb-torrent'),
